      ...
      label43: {
         try {
            ...
            int result = JNI.callPPPPI(
               MemoryUtil.memAddress(pBlock), MemoryUtil.memAddress(pSubBlock), MemoryUtil.memAddress(pBuffer), MemoryUtil.memAddress(pLen), PFN_VerQueryValueW
            );
            if (result == 0) {
               var10 = null;
               break label43;
      ...
      try {
         ...
         label114: {
            ...
            try {
               ...
               int result = JNI.callPPI(
                  MemoryUtil.memAddress(lptstrFilename), lpdwHandle.get(), versionInfoLength, versionInfo.address(), PFN_GetFileVersionInfoW
               );
               if (result == 0) {
                  versionInfo.close();
