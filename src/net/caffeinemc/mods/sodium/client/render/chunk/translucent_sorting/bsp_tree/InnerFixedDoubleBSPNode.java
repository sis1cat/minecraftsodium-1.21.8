...
public class InnerFixedDoubleBSPNode extends InnerPartitionBSPNode {
   ...
   InnerFixedDoubleBSPNode(InnerPartitionBSPNode.NodeReuseData reuseData, BSPNode first, BSPNode second) {
      super(reuseData, 0);
      this.first = first;
      this.second = second;
   }
   ...
   static BSPNode buildFromParts(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode, IntArrayList first, IntArrayList second) {
      ...
      return new InnerFixedDoubleBSPNode(prepareNodeReuse(workspace, indexes, depth), firstNode, secondNode);
   }
}
