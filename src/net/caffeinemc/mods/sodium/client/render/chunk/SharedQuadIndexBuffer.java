...
public class SharedQuadIndexBuffer {
   ...
   public SharedQuadIndexBuffer(CommandList commandList, SharedQuadIndexBuffer.IndexType indexType) {
   ...
   private void grow(CommandList commandList, int primitiveCount) {
      int bufferSize = primitiveCount * this.indexType.getBytesPerElement() * 6;
      commandList.allocateStorage(this.buffer, bufferSize, GlBufferUsage.STATIC_DRAW);
      GlBufferMapping mapped = commandList.mapBuffer(
         this.buffer, 0L, bufferSize, EnumBitField.of(GlBufferMapFlags.INVALIDATE_BUFFER, GlBufferMapFlags.WRITE, GlBufferMapFlags.UNSYNCHRONIZED)
      );
   ...
   public SharedQuadIndexBuffer.IndexType getIndexType() {
      return this.indexType;
   }

   public static enum IndexType {
      SHORT(GlIndexType.UNSIGNED_SHORT, 65536) {
         @Override
      ...
      public static final SharedQuadIndexBuffer.IndexType[] VALUES = values();
      private final GlIndexType format;
      private final int maxElementCount;

      private IndexType(GlIndexType format, int maxElementCount) {
         this.format = format;
