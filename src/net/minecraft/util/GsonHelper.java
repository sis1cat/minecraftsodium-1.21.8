...
public class GsonHelper {
	...
	public static <T> T fromJson(Gson gson, Reader reader, Class<T> class_) {
		try {
			...
			T object = gson.<T>getAdapter(class_).read(jsonReader);
			if (object == null) {
	...
	public static <T> T fromJson(Gson gson, Reader reader, TypeToken<T> typeToken) {
		T object = fromNullableJson(gson, reader, typeToken);
		if (object == null) {
			throw new JsonParseException("JSON data was null or empty");
		} else {
			return object;
	...
	public static <T> T fromNullableJson(Gson gson, String string, TypeToken<T> typeToken) {
		return fromNullableJson(gson, new StringReader(string), typeToken);
	}

	public static <T> T fromJson(Gson gson, String string, Class<T> class_) {
		return fromJson(gson, new StringReader(string), class_);
	}
	...
	public static boolean encodesLongerThan(JsonElement jsonElement, int i) {
		try {
			Streams.write(jsonElement, new JsonWriter(Streams.writerForAppendable(new GsonHelper.CountedAppendable(i))));
			return false;
