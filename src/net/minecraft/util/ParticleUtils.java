...
public class ParticleUtils {
	...
	public static void spawnParticleOnFace(Level level, BlockPos blockPos, Direction direction, ParticleOptions particleOptions, Vec3 vec3, double d) {
		...
		double e = vec32.x + (i == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : i * d);
		double f = vec32.y + (j == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : j * d);
		double g = vec32.z + (k == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : k * d);
	...
	public static void spawnParticles(LevelAccessor levelAccessor, BlockPos blockPos, int i, double d, double e, boolean bl, ParticleOptions particleOptions) {
		...
		for (int j = 0; j < i; j++) {
			...
			double k = 0.5 - d;
			double l = blockPos.getX() + k + randomSource.nextDouble() * d * 2.0;
			double m = blockPos.getY() + randomSource.nextDouble() * e;
			double n = blockPos.getZ() + k + randomSource.nextDouble() * d * 2.0;
	...
	public static void spawnSmashAttackParticles(LevelAccessor levelAccessor, BlockPos blockPos, int i) {
		...
		for (int j = 0; j < i / 1.5F; j++) {
			double d = vec3.x + 3.5 * Math.cos(j) + levelAccessor.getRandom().nextGaussian() / 2.0;
			double e = vec3.y;
			double f = vec3.z + 3.5 * Math.sin(j) + levelAccessor.getRandom().nextGaussian() / 2.0;
