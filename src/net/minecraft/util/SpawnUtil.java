	...
	public static <T extends Mob> Optional<T> trySpawnMob(
		...
		for (int l = 0; l < i; l++) {
			...
			if (serverLevel.getWorldBorder().isWithinBounds(mutableBlockPos)
				&& moveToPossibleSpawnPosition(serverLevel, k, mutableBlockPos, strategy)
				&& (!bl || serverLevel.noCollision(entityType.getSpawnAABB(mutableBlockPos.getX() + 0.5, mutableBlockPos.getY(), mutableBlockPos.getZ() + 0.5)))) {
