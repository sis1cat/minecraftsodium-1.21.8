...
public abstract class ItemStackTagFix extends DataFix {
	...
	public static UnaryOperator<Typed<?>> createFixer(Type<?> type, Predicate<String> predicate, UnaryOperator<Typed<?>> unaryOperator) {
		...
		return typed -> {
			...
			return optional.isPresent() && predicate.test((String)((Pair)optional.get()).getSecond()) ? typed.updateTyped(opticFinder2, unaryOperator) : typed;
		};
	}

	protected abstract Typed<?> fixItemStackTag(Typed<?> typed);
}
