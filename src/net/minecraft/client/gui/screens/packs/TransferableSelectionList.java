...
public class TransferableSelectionList extends ObjectSelectionList<TransferableSelectionList.PackEntry> {
	...
	public TransferableSelectionList(Minecraft minecraft, PackSelectionScreen packSelectionScreen, int i, int j, Component component) {
		super(minecraft, i, j, 33, 36, (int)(9.0F * 1.5F));
	...
	protected void renderHeader(GuiGraphics guiGraphics, int i, int j) {
		...
		guiGraphics.drawString(this.minecraft.font, component, i + this.width / 2 - this.minecraft.font.width(component) / 2, Math.min(this.getY() + 3, j), -1);
	}
	...
	public static class PackEntry extends ObjectSelectionList.Entry<TransferableSelectionList.PackEntry> {
		...
		public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			...
			guiGraphics.blit(RenderPipelines.GUI_TEXTURED, this.pack.getIconTexture(), k, j, 0.0F, 0.0F, 32, 32, 32, 32);
			...
			if (this.showHoverOverlay() && (this.minecraft.options.touchscreen().get() || bl || this.parent.getSelected() == this && this.parent.isFocused())) {
				...
				if (this.pack.canSelect()) {
					if (q < 32) {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.SELECT_HIGHLIGHTED_SPRITE, k, j, 32, 32);
					} else {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.SELECT_SPRITE, k, j, 32, 32);
					}
				} else {
					if (this.pack.canUnselect()) {
						if (q < 16) {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.UNSELECT_HIGHLIGHTED_SPRITE, k, j, 32, 32);
						} else {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.UNSELECT_SPRITE, k, j, 32, 32);
						}
					}

					if (this.pack.canMoveUp()) {
						if (q < 32 && q > 16 && r < 16) {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.MOVE_UP_HIGHLIGHTED_SPRITE, k, j, 32, 32);
						} else {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.MOVE_UP_SPRITE, k, j, 32, 32);
						}
					}

					if (this.pack.canMoveDown()) {
						if (q < 32 && q > 16 && r > 16) {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.MOVE_DOWN_HIGHLIGHTED_SPRITE, k, j, 32, 32);
						} else {
							guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, TransferableSelectionList.MOVE_DOWN_SPRITE, k, j, 32, 32);
						}
					}
				}
			}

			guiGraphics.drawString(this.minecraft.font, formattedCharSequence, k + 32 + 2, j + 1, -1);
		...
		public boolean mouseClicked(double d, double e, int i) {
			double f = d - this.parent.getRowLeft();
			double g = e - this.parent.getRowTop(this.parent.children().indexOf(this));
