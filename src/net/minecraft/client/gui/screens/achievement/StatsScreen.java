	...
	class ItemStatisticsList extends ObjectSelectionList<StatsScreen.ItemStatisticsList.ItemRow> {
		...
		protected void renderHeader(GuiGraphics guiGraphics, int i, int j) {
			...
			for (int k = 0; k < this.iconSprites.length; k++) {
				...
				guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocation, i + this.getColumnX(k) - 18, j + 1, 18, 18);
			}
			...
			if (this.sortColumn != null) {
				...
				guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, resourceLocation, i + k, j + 1, 18, 18);
			}

			for (int k = 0; k < this.iconSprites.length; k++) {
				int l = this.headerPressed == k ? 1 : 0;
				guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.iconSprites[k], i + this.getColumnX(k) - 18 + l, j + 1 + l, 18, 18);
			}
		}
		...
		public boolean mouseClicked(double d, double e, int i) {
			...
			return !bl
					&& this.clickedHeader((int)(d - (this.getX() + this.width / 2.0 - this.getRowWidth() / 2.0)), (int)(e - this.getY()) + (int)this.scrollAmount() - 4)
				? true
				: bl;
		}
		...
		protected void renderDecorations(GuiGraphics guiGraphics, int i, int j) {
			if (j >= this.getY() && j <= this.getBottom()) {
				...
				if (itemRow != null) {
					...
					guiGraphics.setTooltipForNextFrame(StatsScreen.this.font, item.getName(), i, j, item.components().get(DataComponents.TOOLTIP_STYLE));
				} else {
					...
					if (component != null) {
						guiGraphics.setTooltipForNextFrame(StatsScreen.this.font, component, i, j);
					}
				}
			}
		}
		...
		class ItemRow extends ObjectSelectionList.Entry<StatsScreen.ItemStatisticsList.ItemRow> {
			...
			@Override
			public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
				guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, StatsScreen.SLOT_SPRITE, k, j, 18, 18);
