...
public class ServerSelectionList extends ObjectSelectionList<ServerSelectionList.Entry> {
	...
	public static class LANHeader extends ServerSelectionList.Entry {
		...
		public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			...
			guiGraphics.drawString(this.minecraft.font, string, this.minecraft.screen.width / 2 - this.minecraft.font.width(string) / 2, p + 9, -8355712);
		}
	...
	public static class NetworkServerEntry extends ServerSelectionList.Entry {
		...
		@Override
		public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			guiGraphics.drawString(this.minecraft.font, LAN_SERVER_HEADER, k + 32 + 3, j + 1, -1);
			guiGraphics.drawString(this.minecraft.font, this.serverData.getMotd(), k + 32 + 3, j + 12, -8355712);
			if (this.minecraft.options.hideServerAddress) {
				guiGraphics.drawString(this.minecraft.font, HIDDEN_ADDRESS_TEXT, k + 32 + 3, j + 12 + 11, -13619152);
			} else {
				guiGraphics.drawString(this.minecraft.font, this.serverData.getAddress(), k + 32 + 3, j + 12 + 11, -13619152);
			}
		}
	...
	public class OnlineServerEntry extends ServerSelectionList.Entry {
		...
		public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			...
			guiGraphics.drawString(this.minecraft.font, this.serverData.name, k + 32 + 3, j + 1, -1);
			...
			for (int p = 0; p < Math.min(list.size(), 2); p++) {
				guiGraphics.drawString(this.minecraft.font, (FormattedCharSequence)list.get(p), k + 32 + 3, j + 12 + 9 * p, -8355712);
			}

			this.drawIcon(guiGraphics, k, j, this.icon.textureLocation());
			if (this.serverData.state() == ServerData.State.PINGING) {
				int p = (int)(Util.getMillis() / 100L + i * 2 & 7L);
				if (p > 4) {
					p = 8 - p;
				}
			...
			if (this.statusIcon != null) {
				guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.statusIcon, p, j, 10, 8);
			}
			...
			int r = p - q - 5;
			guiGraphics.drawString(this.minecraft.font, component, r, j + 1, -8355712);
			if (this.statusIconTooltip != null && n >= p && n <= p + 10 && o >= j && o <= j + 8) {
				guiGraphics.setTooltipForNextFrame(this.statusIconTooltip, n, o);
			} else if (this.onlinePlayersTooltip != null && n >= r && n <= r + q && o >= j && o <= j - 1 + 9) {
				guiGraphics.setTooltipForNextFrame(Lists.transform(this.onlinePlayersTooltip, Component::getVisualOrderText), n, o);
			}
			...
			if (this.minecraft.options.touchscreen().get() || bl) {
				...
				if (this.canJoin()) {
					if (s < 32 && s > 16) {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.JOIN_HIGHLIGHTED_SPRITE, k, j, 32, 32);
					} else {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.JOIN_SPRITE, k, j, 32, 32);
					}
				}

				if (i > 0) {
					if (s < 16 && t < 16) {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_UP_HIGHLIGHTED_SPRITE, k, j, 32, 32);
					} else {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_UP_SPRITE, k, j, 32, 32);
					}
				}

				if (i < this.screen.getServers().size() - 1) {
					if (s < 16 && t > 16) {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_DOWN_HIGHLIGHTED_SPRITE, k, j, 32, 32);
					} else {
						guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_DOWN_SPRITE, k, j, 32, 32);
					}
				}
			}
		}
		...
		@Override
		public boolean mouseClicked(double d, double e, int i) {
			double f = d - ServerSelectionList.this.getRowLeft();
			double g = e - ServerSelectionList.this.getRowTop(ServerSelectionList.this.children().indexOf(this));
			if (f <= 32.0) {
