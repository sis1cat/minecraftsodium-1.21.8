...
public class TabNavigationBar extends AbstractContainerEventHandler implements Renderable, NarratableEntry {
	...
	private int getNextTabIndex(int i) {
		return this.getNextTabIndex(this.currentTabIndex(), i);
	}

	private int getNextTabIndex(int i, int j) {
		if (j >= 49 && j <= 57) {
			return j - 49;
		} else if (j == 258 && i != -1) {
			int k = Screen.hasShiftDown() ? i - 1 : i + 1;
			int l = Math.floorMod(k, this.tabs.size());
			return ((TabButton)this.tabButtons.get(l)).active ? l : this.getNextTabIndex(l, j);
		} else {
			return -1;
		}
	}
