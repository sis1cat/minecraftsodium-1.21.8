		...
		if (level == null) {
			...
			for (GameEventListenerRenderer.TrackedListener trackedListener : this.trackedListeners) {
				trackedListener.getPosition(level).ifPresent(vec3x -> {
					double gx = vec3x.x() - trackedListener.getListenerRadius();
					double hx = vec3x.y() - trackedListener.getListenerRadius();
					double ix = vec3x.z() - trackedListener.getListenerRadius();
					double jx = vec3x.x() + trackedListener.getListenerRadius();
					double kx = vec3x.y() + trackedListener.getListenerRadius();
					double lx = vec3x.z() + trackedListener.getListenerRadius();
