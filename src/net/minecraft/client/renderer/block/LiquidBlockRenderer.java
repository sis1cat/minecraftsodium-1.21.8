...
public class LiquidBlockRenderer {
	...
	public void tesselate(BlockAndTintGetter blockAndTintGetter, BlockPos blockPos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
		...
		if (info.handler == null) {
			...
			if (handler != null) {
				handler.renderFluid(blockPos, blockAndTintGetter, vertexConsumer, blockState, fluidState, blockPos.getX(), blockPos.getY(), blockPos.getZ());
				return;
			}
		}
		...
		if (info.handler != null) {
			textureAtlasSprites = info.sprites;
		}
		int i = bl ? 16777215 : BiomeColors.getAverageWaterColor(blockAndTintGetter, blockPos);
		if (info.handler != null) {
			i = info.handler.getFluidColor(blockAndTintGetter, blockPos, fluidState);
		}
		...
		if (bl2 || bl3 || bl7 || bl6 || bl4 || bl5) {
			...
			for (Direction direction : Direction.Plane.HORIZONTAL) {
				...
				if (bl8 && !isFaceOccludedByNeighbor(direction, Math.max(adx, yx), blockAndTintGetter.getBlockState(blockPos.relative(direction)))) {
					...
					TextureAtlasSprite overlaySprite = this.waterOverlay;
					if (info.handler != null && info.hasOverlay) {
						overlaySprite = info.overlaySprite;
					}
					if (!bl) {
						...
						if (FluidRenderHandlerRegistry.INSTANCE.isBlockTransparent(block)) {
							textureAtlasSprite2 = overlaySprite;
						}
					}
