...
public class DecoratedPotRenderer implements BlockEntityRenderer<DecoratedPotBlockEntity> {
	...
	public void render(DecoratedPotBlockEntity decoratedPotBlockEntity, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j, Vec3 vec3) {
		...
		if (wobbleStyle != null && decoratedPotBlockEntity.getLevel() != null) {
			float g = ((float)(decoratedPotBlockEntity.getLevel().getGameTime() - decoratedPotBlockEntity.wobbleStartedAtTick) + f) / wobbleStyle.duration;
			if (g >= 0.0F && g <= 1.0F) {
	...
	public void renderInHand(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j, PotDecorations potDecorations) {
		this.render(poseStack, multiBufferSource, i, j, potDecorations);
	}

	private void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j, PotDecorations potDecorations) {
