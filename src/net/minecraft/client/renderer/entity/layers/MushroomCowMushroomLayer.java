...
public class MushroomCowMushroomLayer extends RenderLayer<MushroomCowRenderState, CowModel> {
	...
	public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, MushroomCowRenderState mushroomCowRenderState, float f, float g) {
		if (!mushroomCowRenderState.isBaby) {
			...
			if (!mushroomCowRenderState.isInvisible || bl) {
				...
				this.renderMushroomBlock(poseStack, multiBufferSource, i, bl, blockState, j, blockStateModel);
				poseStack.popPose();
				poseStack.pushPose();
				poseStack.translate(0.2F, -0.35F, 0.5F);
	...
	private void renderProxy(PoseStack.Pose matrices, VertexConsumer vertexConsumer, BlockStateModel model, int light, int overlay, BlockState mushroomState) {
	...
	private void renderMushroomBlock(
		...
		if (bl) {
			renderProxy(poseStack.last(), multiBufferSource.getBuffer(RenderType.outline(TextureAtlas.LOCATION_BLOCKS)), blockStateModel, i, j, blockState);
			/*ModelBlockRenderer.renderModel(
				poseStack.last(), multiBufferSource.getBuffer(RenderType.outline(TextureAtlas.LOCATION_BLOCKS)), blockStateModel, 0.0F, 0.0F, 0.0F, i, j
			);*/
		} else {
