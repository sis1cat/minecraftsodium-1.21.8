	...
	public GrindstoneMenu(int i, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
		...
		this.addSlot(new Slot(this.resultSlots, 2, 129, 34) {
			...
			private int getExperienceAmount(Level level) {
				int ix = 0;
				ix += this.getExperienceFromItem(GrindstoneMenu.this.repairSlots.getItem(0));
				ix += this.getExperienceFromItem(GrindstoneMenu.this.repairSlots.getItem(1));
				if (ix > 0) {
					int j = (int)Math.ceil(ix / 2.0);
