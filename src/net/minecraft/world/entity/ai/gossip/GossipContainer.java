...
public class GossipContainer {
	...
	private GossipContainer(List<GossipContainer.GossipEntry> list) {
		list.forEach(gossipEntry -> this.getOrCreate(gossipEntry.target).entries.put(gossipEntry.type, gossipEntry.value));
	}
	...
	public long getCountForType(GossipType gossipType, DoublePredicate doublePredicate) {
		return this.gossips
			.values()
			.stream()
			.filter(entityGossips -> doublePredicate.test(entityGossips.entries.getOrDefault(gossipType, 0) * gossipType.weight))
			.count();
	}
	...
	public void putAll(GossipContainer gossipContainer) {
		gossipContainer.gossips.forEach((uUID, entityGossips) -> this.getOrCreate(uUID).entries.putAll(entityGossips.entries));
	}
