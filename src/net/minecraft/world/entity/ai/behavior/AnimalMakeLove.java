...
public class AnimalMakeLove extends Behavior<Animal> {
	...
	private Optional<? extends Animal> findValidBreedPartner(Animal animal) {
		return ((NearestVisibleLivingEntities)animal.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get())
			.findClosest(
				livingEntity -> livingEntity.getType() == this.partnerType && livingEntity instanceof Animal animal2 && animal.canMate(animal2) && !animal2.isPanicking()
			)
