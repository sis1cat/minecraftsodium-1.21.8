				...
				.apply(
					...
					(memoryAccessor, memoryAccessor2, memoryAccessor3, memoryAccessor4, memoryAccessor5, memoryAccessor6, memoryAccessor7) -> (serverLevel, livingEntity, l) -> {
						serverLevel.getPoiManager()
							...
							.filter(blockPos -> blockPos.closerToCenterThan(livingEntity.position(), j))
							.or(
								() -> serverLevel.getPoiManager()
									.getRandom(holder -> holder.is(PoiTypes.HOME), blockPos -> true, PoiManager.Occupancy.ANY, livingEntity.blockPosition(), i, livingEntity.getRandom())
							)
