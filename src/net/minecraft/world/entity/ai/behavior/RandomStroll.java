...
public class RandomStroll {
	...
	public static OneShot<PathfinderMob> stroll(float f, boolean bl) {
		return strollFlyOrSwim(
			f, pathfinderMob -> LandRandomPos.getPos(pathfinderMob, 10, 7), bl ? pathfinderMob -> true : pathfinderMob -> !pathfinderMob.isInWater()
		);
	}
	...
	public static BehaviorControl<PathfinderMob> swim(float f) {
		return strollFlyOrSwim(f, RandomStroll::getTargetSwimPos, Entity::isInWater);
	}
	...
	private static Vec3 getTargetSwimPos(PathfinderMob pathfinderMob) {
		...
		for (int[] is : SWIM_XY_DISTANCE_TIERS) {
			if (vec3 == null) {
				...
				vec32 = pathfinderMob.position().add(pathfinderMob.position().vectorTo(vec3).normalize().multiply(is[0], is[1], is[0]));
			}
