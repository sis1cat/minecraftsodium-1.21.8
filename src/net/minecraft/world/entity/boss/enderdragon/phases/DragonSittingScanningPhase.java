	...
	public void doServerTick(ServerLevel serverLevel) {
		...
		if (livingEntity != null) {
			if (this.scanningTime > 25) {
				...
				float g = (float)(Math.acos(f) * 180.0F / (float)Math.PI) + 0.5F;
				if (g < 0.0F || g > 10.0F) {
					double d = livingEntity.getX() - this.dragon.head.getX();
					double e = livingEntity.getZ() - this.dragon.head.getZ();
					double h = Mth.clamp(Mth.wrapDegrees(180.0 - Mth.atan2(d, e) * 180.0F / (float)Math.PI - this.dragon.getYRot()), -100.0, 100.0);
