...
public class DripstoneClusterFeature extends Feature<DripstoneClusterConfiguration> {
	...
	private int getDripstoneHeight(RandomSource randomSource, int i, int j, float f, int k, DripstoneClusterConfiguration dripstoneClusterConfiguration) {
		if (randomSource.nextFloat() > f) {
			...
			float g = (float)Mth.clampedMap((double)l, 0.0, (double)dripstoneClusterConfiguration.maxDistanceFromCenterAffectingHeightBias, k / 2.0, 0.0);
			return (int)randomBetweenBiased(randomSource, 0.0F, k, g, dripstoneClusterConfiguration.heightDeviation);
		}
	}
