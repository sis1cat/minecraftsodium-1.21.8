...
public class FancyTrunkPlacer extends TrunkPlacer {
	...
	public List<FoliagePlacer.FoliageAttachment> placeTrunk(
		...
		for (; o >= 0; o--) {
			...
			if (!(f < 0.0F)) {
				for (int p = 0; p < m; p++) {
					double e = 1.0;
					double g = 1.0 * f * (randomSource.nextFloat() + 0.328);
					double h = randomSource.nextFloat() * 2.0F * Math.PI;
					...
					if (this.makeLimb(levelSimulatedReader, biConsumer, randomSource, blockPos2, blockPos3, false, treeConfiguration)) {
						...
						double u = blockPos2.getY() - Math.sqrt(s * s + t * t) * 0.381;
						int v = u > n ? n : (int)u;
	...
	private boolean makeLimb(
		...
		if (!bl && Objects.equals(blockPos, blockPos2)) {
			...
			float f = (float)blockPos3.getX() / i;
			float g = (float)blockPos3.getY() / i;
			float h = (float)blockPos3.getZ() / i;

			for (int j = 0; j <= i; j++) {
				BlockPos blockPos4 = blockPos.offset(Mth.floor(0.5F + j * f), Mth.floor(0.5F + j * g), Mth.floor(0.5F + j * h));
				if (bl) {
					this.placeLog(
