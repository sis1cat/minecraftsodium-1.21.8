...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
	...
	private void generateIcebergBlock(
		...
		if (e < 0.0) {
			...
			double f = bl ? -0.5 : -6 - randomSource.nextInt(3);
	...
	private void setIcebergBlock(
		...
		if (blockState2.isAir() || blockState2.is(Blocks.SNOW_BLOCK) || blockState2.is(Blocks.ICE) || blockState2.is(Blocks.WATER)) {
			...
			int k = bl ? 3 : 2;
			if (bl2 && !blockState2.is(Blocks.WATER) && i <= randomSource.nextInt(Math.max(1, j / k)) + j * 0.6 && bl3) {
	...
	private double signedDistanceCircle(int i, int j, BlockPos blockPos, int k, RandomSource randomSource) {
		float f = 10.0F * Mth.clamp(randomSource.nextFloat(), 0.2F, 0.8F) / k;
		return f + Math.pow(i - blockPos.getX(), 2.0) + Math.pow(j - blockPos.getZ(), 2.0) - Math.pow(k, 2.0);
	}

	private double signedDistanceEllipse(int i, int j, BlockPos blockPos, int k, int l, double d) {
		return Math.pow(((i - blockPos.getX()) * Math.cos(d) - (j - blockPos.getZ()) * Math.sin(d)) / k, 2.0)
			+ Math.pow(((i - blockPos.getX()) * Math.sin(d) + (j - blockPos.getZ()) * Math.cos(d)) / l, 2.0)
			- 1.0;
	}

	private int heightDependentRadiusRound(RandomSource randomSource, int i, int j, int k) {
		float f = 3.5F - randomSource.nextFloat();
		float g = (1.0F - (float)Math.pow(i, 2.0) / (j * f)) * k;
	...
	private int heightDependentRadiusEllipse(int i, int j, int k) {
		float f = 1.0F;
		float g = (1.0F - (float)Math.pow(i, 2.0) / (j * 1.0F)) * k;
		return Mth.ceil(g / 2.0F);
	}
