...
public class SurfaceSystem {
	...
	private void erodedBadlandsExtension(BlockColumn blockColumn, int i, int j, int k, LevelHeightAccessor levelHeightAccessor) {
		double d = 0.2;
		double e = Math.min(Math.abs(this.badlandsSurfaceNoise.getValue(i, 0.0, j) * 8.25), this.badlandsPillarNoise.getValue(i * 0.2, 0.0, j * 0.2) * 15.0);
		if (!(e <= 0.0)) {
			double f = 0.75;
			double g = 1.5;
			double h = Math.abs(this.badlandsPillarRoofNoise.getValue(i * 0.75, 0.0, j * 0.75) * 1.5);
	...
	private void frozenOceanExtension(int i, Biome biome, BlockColumn blockColumn, BlockPos.MutableBlockPos mutableBlockPos, int j, int k, int l) {
		double d = 1.28;
		double e = Math.min(Math.abs(this.icebergSurfaceNoise.getValue(j, 0.0, k) * 8.25), this.icebergPillarNoise.getValue(j * 1.28, 0.0, k * 1.28) * 15.0);
		if (!(e <= 1.8)) {
			double f = 1.17;
			double g = 1.5;
			double h = Math.abs(this.icebergPillarRoofNoise.getValue(j * 1.17, 0.0, k * 1.17) * 1.5);
