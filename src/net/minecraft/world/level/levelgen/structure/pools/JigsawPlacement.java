...
public class JigsawPlacement {
	...
	public static Optional<Structure.GenerationStub> addPieces(
		...
		if (structurePoolElement == EmptyPoolElement.INSTANCE) {
			...
			int m = optional2.isEmpty()
				? blockPos3.getY()
				: blockPos.getY() + chunkGenerator.getFirstFreeHeight(k, l, (Heightmap.Types)optional2.get(), levelHeightAccessor, generationContext.randomState());
					...
					new Structure.GenerationStub(
						...
						structurePiecesBuilder -> {
							...
							if (i > 0) {
								AABB aABB = new AABB(
									k - j,
									Math.max(o - j, levelHeightAccessor.getMinY() + dimensionPadding.bottom()),
									l - j,
									k + j + 1,
									Math.min(o + j + 1, levelHeightAccessor.getMaxY() + 1 - dimensionPadding.top()),
									l + j + 1
								);
