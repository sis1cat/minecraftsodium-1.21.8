...
public class FrogspawnBlock extends Block {
	...
	private void spawnTadpoles(ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
		...
		for (int j = 1; j <= i; j++) {
			...
			if (tadpole != null) {
				double d = blockPos.getX() + this.getRandomTadpolePositionOffset(randomSource);
				double e = blockPos.getZ() + this.getRandomTadpolePositionOffset(randomSource);
				int k = randomSource.nextInt(1, 361);
				tadpole.snapTo(d, blockPos.getY() - 0.5, e, k, 0.0F);
