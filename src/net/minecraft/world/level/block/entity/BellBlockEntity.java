		...
		list.stream()
			...
			.forEach(
				livingEntity -> {
					...
					double e = blockPos.getX() + 0.5F + 1.0 / d * (livingEntity.getX() - blockPos.getX());
					double g = blockPos.getZ() + 0.5F + 1.0 / d * (livingEntity.getZ() - blockPos.getZ());
					int j = Mth.clamp((i - 21) / -2, 3, 15);

					for (int k = 0; k < j; k++) {
						int l = mutableInt.addAndGet(5);
						level.addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, l), e, blockPos.getY() + 0.5F, g, 0.0, 0.0, 0.0);
					}
				}
			);
	}
