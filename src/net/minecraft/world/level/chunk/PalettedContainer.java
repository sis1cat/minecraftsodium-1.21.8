...
public class PalettedContainer<T> implements PaletteResize<T>, PalettedContainerRO<T>, PalettedContainerROExtension<T> {
	...
	public void sodium$unpack(T[] values) {
		Strategy strategy = Objects.requireNonNull(this.strategy);
		if (values.length != strategy.size()) {
			throw new IllegalArgumentException("Array is wrong size");
		} else {
			Data<T> data = Objects.requireNonNull(this.data, "PalettedContainer must have data");
			...
			for (int y = minY; y <= maxY; y++) {
				for (int z = minZ; z <= maxZ; z++) {
					for (int x = minX; x <= maxX; x++) {
						...
						T paletteValue = (T)palette.valueFor(paletteIndex);
