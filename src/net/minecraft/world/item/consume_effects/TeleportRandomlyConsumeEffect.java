...
public record TeleportRandomlyConsumeEffect(float diameter) implements ConsumeEffect {
	...
	public boolean apply(Level level, ItemStack itemStack, LivingEntity livingEntity) {
		...
		for (int i = 0; i < 16; i++) {
			double d = livingEntity.getX() + (livingEntity.getRandom().nextDouble() - 0.5) * this.diameter;
			double e = Mth.clamp(
				livingEntity.getY() + (livingEntity.getRandom().nextDouble() - 0.5) * this.diameter,
				(double)level.getMinY(),
				(double)(level.getMinY() + ((ServerLevel)level).getLogicalHeight() - 1)
			);
			double f = livingEntity.getZ() + (livingEntity.getRandom().nextDouble() - 0.5) * this.diameter;
