...
public record SpawnParticlesEffect(
	...
	public void apply(ServerLevel serverLevel, int i, EnchantedItemInUse enchantedItemInUse, Entity entity, Vec3 vec3) {
		...
		serverLevel.sendParticles(
			...
			this.verticalPosition.getCoordinate(vec3.y(), vec3.y() + g / 2.0F, g, randomSource),
	...
	public record PositionSource(SpawnParticlesEffect.PositionSourceType type, float offset, float scale) {
		...
		public double getCoordinate(double d, double e, float f, RandomSource randomSource) {
			return this.type.getCoordinate(d, e, f * this.scale, randomSource) + this.offset;
		}
	}

	public static enum PositionSourceType implements StringRepresentable {
		ENTITY_POSITION("entity_position", (d, e, f, randomSource) -> d),
		BOUNDING_BOX("in_bounding_box", (d, e, f, randomSource) -> e + (randomSource.nextDouble() - 0.5) * f);
