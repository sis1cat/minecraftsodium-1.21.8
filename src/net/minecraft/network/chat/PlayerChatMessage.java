...
public record PlayerChatMessage(
	...
	public static String describeSigned(PlayerChatMessage playerChatMessage) {
		return "'"
			+ playerChatMessage.signedBody.content()
			+ "' @ "
			...
			+ "\n - Salt: "
			+ playerChatMessage.signedBody.salt()
			+ "\n - Signature: "
			+ MessageSignature.describe(playerChatMessage.signature)
			+ "\n - Last Seen: [\n"
			+ (String)playerChatMessage.signedBody
				.lastSeen()
				.entries()
				.stream()
				.map(messageSignature -> "     " + MessageSignature.describe(messageSignature) + "\n")
