...
public record WrappedMinMaxBounds(@Nullable Float min, @Nullable Float max) {
	...
	public boolean matchesSqr(double d) {
		if (this.min != null && this.max != null && this.min > this.max && this.min * this.min > d && this.max * this.max < d) {
			return false;
		} else {
			return this.min != null && this.min * this.min > d ? false : this.max == null || !(this.max * this.max < d);
		}
	}
