...
public class PlacementUtils {
	...
	public static PlacementModifier countExtra(int i, float f, int j) {
		float g = 1.0F / f;
		if (Math.abs(g - (int)g) > 1.0E-5F) {
			throw new IllegalStateException("Chance data cannot be represented as list weight");
		} else {
			WeightedList<IntProvider> weightedList = WeightedList.<IntProvider>builder().add(ConstantInt.of(i), (int)g - 1).add(ConstantInt.of(i + j), 1).build();
