...
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
	...
	class Entry extends ObjectSelectionList.Entry<RealmsSelectWorldTemplateScreen.Entry> {
		...
		public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
			guiGraphics.blit(
				RenderPipelines.GUI_TEXTURED, RealmsTextureManager.worldTemplate(this.template.id, this.template.image), k + 1, j + 1 + 1, 0.0F, 0.0F, 38, 38, 38, 38
			);
			guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, RealmsSelectWorldTemplateScreen.SLOT_FRAME_SPRITE, k, j + 1, 40, 40);
			int p = 5;
			...
			int r = k + 45 + 20;
			int s = j + 5;
			guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.name, r, s, -1);
			guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.version, k + l - q - 5, s, -9671572);
			guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.author, r, s + 9 + 5, -6250336);
			if (!this.template.recommendedPlayers.isBlank()) {
				guiGraphics.drawString(RealmsSelectWorldTemplateScreen.this.font, this.template.recommendedPlayers, r, j + m - 9 / 2 - 5, -11776948);
			}
		}
