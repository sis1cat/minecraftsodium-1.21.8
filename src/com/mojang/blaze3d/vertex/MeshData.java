...
public class MeshData implements AutoCloseable {
	...
	private static Vector3f[] unpackQuadCentroids(ByteBuffer byteBuffer, int i, VertexFormat vertexFormat) {
		...
		int vertexStride = vertexFormat.getVertexSize();
		int primitiveCount = i / 4;
		Vector3f[] centers = new Vector3f[primitiveCount];

		for (int index = 0; index < primitiveCount; index++) {
			long v1 = MemoryUtil.memAddress(byteBuffer, (index * 4 + 0) * vertexStride);
			long v2 = MemoryUtil.memAddress(byteBuffer, (index * 4 + 2) * vertexStride);
